...
st.session_state.data = recalc(edited_df)
...
with col1:
    if st.button("Add Row"):
        st.session_state.data = pd.concat([st.session_state.data, pd.DataFrame([[""]*14], columns=st.session_state.data.columns)], ignore_index=True)

with col2:
    if st.button("Save Locally"):
        st.success("Data saved locally (session)")
