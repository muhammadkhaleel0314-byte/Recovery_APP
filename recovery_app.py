...
with st.sidebar.expander("📊 MDP Report"):
    ...
    if active_file and mdp_file:
        ...
        # ---------------- RECOVERY SECTION (Sidebar under MDP) ----------------
with st.sidebar.expander("📊 Recovery Summary", expanded=False):

    st.markdown("### Upload Recovery File")

    uploaded = st.file_uploader(
        "Upload Recovery Excel / CSV",
        type=["xlsx", "csv"],
        key="recovery_upload"
    )
    ...
    # ---------- Column Selection ----------
    date_col = st.selectbox("Date Column", df.columns, key="rec_date")
    branch_col = st.selectbox("Branch Column", df.columns, key="rec_branch")
...
if "df" in st.session_state:
    ...
    st.download_button("⬇ Download CSV", csv, "recovery_summary.csv")
    ...
    st.download_button("⬇ Download PDF", pdf_bytes, "recovery.pdf")
import streamlit as st
import pandas as pd
from io import BytesIO
